...
export default function NovaOferta() {
  ...
  const percentualDesconto = useMemo(() => {
    if (!produtoSelecionado || !precoPromocional) {
      return null;
    }
    ...
    if (!Number.isFinite(original) || !Number.isFinite(promocional) || original <= 0) {
      return null;
    }

    return Math.max(0, Math.round(((original - promocional) / original) * 100));
  }, [precoPromocional, produtoSelecionado]);
  ...
  const carregarProdutos = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Erro ${response.status}: não foi possível carregar os produtos.`);
      }
  ...
  useEffect(() => {
    ...
    if (produtoSelecionadoViaUrl) {
      setProdutoId(produtoSelecionadoViaUrl);
    }
  ...
  async function handleCriarOferta(e) {
    ...
    if (!validadeProduto || !dataFimOferta) {
      setErro('Preencha a validade do produto e a data fim da oferta.');
      return;
    }
      ...
      <div className="row g-0 flex-grow-1" style={{ overflow: 'hidden' }}>
        ...
        <main className="col-md-9 ms-sm-auto col-lg-10 px-md-4 p-4" style={{ height: '100vh', overflowY: 'auto', backgroundColor: '#f8f9fa' }}>
          <div className="d-flex justify-content-between align-items-center pt-3 pb-2 mb-4 border-bottom">
            ...
            <Link to="/ofertas" className="btn btn-light rounded-3">
              ← Voltar para ofertas
            </Link>
          </div>
          ...
          {sucesso && (
            <div className="alert alert-success rounded-3" role="alert">
              ✓ Oferta criada com sucesso! Redirecionando...
            </div>
          )}
              ...
              <form onSubmit={handleCriarOferta}>
                ...
                <div className="row g-3 mb-3">
                  <div className="col-md-6">
                    <label className="form-label fw-medium">Validade do produto</label>
                    <input type="date" className="form-control" value={validadeProduto} onChange={(event) => setValidadeProduto(event.target.value)} required />
                  </div>
                  <div className="col-md-6">
                    <label className="form-label fw-medium">Fim da oferta</label>
                    <input type="date" className="form-control" value={dataFimOferta} onChange={(event) => setDataFimOferta(event.target.value)} required />
                  </div>
                </div>
                ...
                <div className="row g-3 mb-3">
                  <div className="col-md-6">
                    ...
                    <input type="number" className="form-control" value={precoPromocional} onChange={(event) => setPrecoPromocional(event.target.value)} min="0.01" step="0.01" required />
                  </div>
                  <div className="col-md-6">
                    <label className="form-label fw-medium">Desconto estimado</label>
                    <input type="text" className="form-control" value={percentualDesconto !== null ? `${percentualDesconto}%` : 'Aguardando preço original'} disabled />
                  </div>
                </div>
                ...
                <div className="alert alert-light rounded-3 border">
                  ...
                  {produtoSelecionado && (
                    <span className="d-block mt-2 text-muted">
                      Preço original atual: R$ {Number(produtoSelecionado.precoOriginal).toFixed(2)}
                    </span>
                  )}
                </div>

                <button type="submit" className="btn w-100 fw-bold py-3 text-white" style={{ backgroundColor: '#3aad77', borderRadius: '10px' }} disabled={loading}>
